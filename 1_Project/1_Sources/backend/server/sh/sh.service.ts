...
export default class ShService {
     ...
     */
     public async signData(folder: string) {
        ...
        // Read file inline instead of using FileService
        const result: string = fs.readFileSync(`${folder}/resultSign.dat`, 'utf8');
    ...
    public async execRestartService() {
        ...
        if (stderr) {
            this.logger.writeLog(this.nameFile, `Fallo al restaurar el servicio`);
            return;
        }
