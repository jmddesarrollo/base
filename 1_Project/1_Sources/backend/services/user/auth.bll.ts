...
export default class AuthService {
    ...
    public async login(userName: string, password: string, t: any) {
        ...
        var token = jwt.sign({ user }, process.env.APP_SEED, { expiresIn: process.env.APP_EXPIRATION_TOKEN });
    ...
    public recoveryToken(req: any) {
        var token = jwt.sign({ user: req.user }, process.env.APP_SEED, { expiresIn: process.env.APP_EXPIRATION_TOKEN_RECOVERY });
